...
$gogger= "https://github.com/xatornet/GridGogger/raw/main/zoom2gog.vcdiff"
...
Clear-Host
# Check if grid.zoom.bak exists
    Write-Host "Checking if Exe file has been patched..."
	Write-Host ""
	Delay 3
if (Test-Path -Path "Grid.zoom.bak") {
	...
	Invoke-WebRequest -Uri $gogger -OutFile ".\zoom2gog.vcdiff"
	Expand-Archive ".\xdelta3.zip" ".\" | Out-Null
	Delay 3
	Rename-Item -Path "Grid.exe" "Grid.zoom.bak"
	Start-Process -Wait -Filepath ".\xdelta3-3.1.0-x86_64.exe" -ArgumentList "-d -s Grid.zoom.bak zoom2gog.vcdiff Grid.exe" | Out-Null
	Write-Host ""
	...
	Remove-Item -Path ".\xdelta3.zip" -Force -ErrorAction SilentlyContinue
		Remove-Item -Path ".\xdelta3-3.1.0-x86_64.exe" -Force -ErrorAction SilentlyContinue
			Remove-Item -Path ".\zoom2gog.vcdiff" -Force -ErrorAction SilentlyContinue
	Delay 3
	Clear-Host
	Write-Host ""
	Write-Host "Loading script..."
